...
export default class Cards {
  constructor(instance, app) {
    ...
    this.path = this.instance
      .querySelector(".curve-path")
      .querySelector("path");

    this.quicks = [...this.cards].map((item, index) => {
      const rotation = gsap.quickTo(item, "rotation", {
        duration: 0.4,
        ease: "power2",
      });
    ...
    this.cards.forEach((item, index) => {
      const centerIndex =
        (1 -
          Math.abs(index - (this.quicks.length - 1) / 2) /
            (this.quicks.length - 1)) /
        2;

      item.addEventListener("mouseenter", () => {
        this.quicks[index].y(
          item.getBoundingClientRect().height * -0.1 * centerIndex
        );
        ...
        onUpdate: () => {
          ...
          this.quicks.forEach(({ rotation, y, x, item }, index) => {
            ...
            const staggeredProgress = Math.max(
              ...
              Math.min(
                1,
                (progress - staggerOffset * this.staggerAmount) /
                  (1 - this.staggerAmount)
              )
            );
            // fil all cards inside the wrapper
            const startX = window.innerWidth;
            const endX =
              startX * (1 - staggeredProgress) +
              offset * centerIndex * staggeredProgress;
            const rotateProgres =
              1 - ((index + 1) / this.quicks.length) * progress;
            ...
            const yValue =
              window.innerHeight * 0.15 * (1 - staggeredProgress) +
              (point.y / 2) * staggeredProgress;
