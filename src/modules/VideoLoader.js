...
export default class VideoLoader extends EventEmitter
{
    constructor(videoElement, app, options = {})
    {
        ...
        if (app && typeof app === 'object' && !app.constructor.name.includes('App')) {
        ...
        this.video = videoElement.tagName === 'VIDEO' ? videoElement : videoElement.querySelector('video')
        ...
        const mobileUrl = this.source.getAttribute('data-src-mobile') || this.source.dataset.srcMobile
        const desktopUrl = this.source.getAttribute('data-src') || this.source.dataset.src
    ...
    detectWebGLVideo() {
        ...
        const webglSelectors = ['.double-video', '.cases_video', '.preview_img', '.talk_full']
        
        // If data-module is on video element, check parent containers
        // If data-module is on wrapper, check the wrapper itself
        const elementToCheck = this.video === this.instance ? this.video.parentElement : this.instance
        
        return webglSelectors.some(selector => {
            const className = selector.substring(1)
            // Check the element itself
            if (elementToCheck && elementToCheck.classList.contains(className)) return true
    ...
    init() {
        ...
        // WebGL videos load immediately, others use lazy loading
        if (this.isWebGLVideo || !this.options.lazyLoad) {
            this.startLoading()
        } else {
    ...
    setupUserInteractionUpgrade() {
        const upgradePreload = () => {
            ...
            document.removeEventListener('touchstart', upgradePreload, { passive: true })
        ...
        document.addEventListener('touchstart', upgradePreload, { passive: true, once: true })
        document.addEventListener('scroll', upgradePreload, { passive: true, once: true })
