...
void main()
{
    ...
    gl_FragColor = color;
}`;r.registerPlugin(n);class U{constructor(e,o,a,h,u,l){this.app=e,this.gl=o,this.scene=a,this.item=h,this.texture=u,this.index=l,this.sizes=this.app.sizes,this.time=this.app.time,this.init()}init(){this.setMaterial(),this.setMesh()}setMaterial(){this.material=new m({vertexShader:g,fragmentShader:f,transparent:!0,depthTest:!1,uniforms:{uTexture:new t(this.texture),uRes:new t(new i(this.sizes.width,this.sizes.height)),uAspect:new t(new i(16,9)),uSize:new t(new i(0,0)),uFluid:new t(null),uColor:new t(new c(255/255,118/255,162/255)),uLoading:new t(0),uTranslateY:new t(1.2)}})}setMesh(){this.rect=this.item.getBoundingClientRect(),this.geometry=new s(this.rect.width,this.rect.height,1,1),this.material.uniforms.uSize.value.set(this.rect.width,this.rect.height),this.material.uniforms.uAspect.value.set(this.rect.width,this.rect.height),this.mesh=new d(this.geometry,this.material),this.mesh.renderOrder=10,this.mesh.rotation.x=-Math.PI/2,this.scene.add(this.mesh),this.setPosition();const e=r.timeline({paused:!0,defaults:{duration:1.4,ease:"power2"}});e.to(this.material.uniforms.uLoading,{value:1},this.index*.3),e.to(this.mesh.rotation,{y:0,x:0},this.index*.3),e.to(this.mesh.position,{z:0},this.index*.3),n.create({trigger:this.item,start:"top 80%",onEnter:()=>e.restart()})}setPosition(){this.rect=this.item.getBoundingClientRect(),this.mesh.position.x=this.rect.left+this.rect.width/2-this.sizes.width/2,this.mesh.position.y=-this.rect.top-this.rect.height/2+this.sizes.height/2}resize(){this.rect=this.item.getBoundingClientRect(),v(this.mesh,new s(this.rect.width,this.rect.height,1,1)),this.mesh.material.uniforms.uSize.value.set(this.rect.width,this.rect.height)}update(){this.material.uniforms.uFluid.value=this.gl.fluidTexture}destroy(){this.material.dispose(),this.geometry.dispose(),this.scene.remove(this.mesh)}}export{U as i};
