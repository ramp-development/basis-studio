import{R as T}from"./Resources--KCJCONz.js";import{i as C}from"./index-Cwx_fOX2.js";import{V as h,S as w,U as r,C as R,G as S,P as g,a as m,M as U,b as A}from"./GL-BGRjnAmH.js";import b from"./VideoLoader-K1Zmm6xk.js";import"./app.js";var P=`uniform vec2 uMouse;
uniform vec2 uRes;
uniform float uTime;
...
void main()
{
    ...
    gl_FragColor = color;
}`;let X=class{constructor(e,o,t,s,i){this.app=e,this.gl=o,this.scene=t,this.main=s,this.resources=i.items,this.sizes=this.app.sizes,this.time=this.app.time,this.items=this.main.querySelectorAll(".hero_image, .hero_video"),this.mouse=new h(0,0),this.offset=new h(0,0),this.outputOffset=new h(0,0),this.mouseEnabled=!1,this.offsetQuicks={x:gsap.quickTo(this.outputOffset,"x",{duration:.6,ease:"power2.out",onUpdate:()=>{this.mouseEnabled&&this.meshs.forEach(({material:n})=>n.uniforms.uMouse.value.set(this.outputOffset.x,this.outputOffset.y))}}),y:gsap.quickTo(this.outputOffset,"y",{duration:.6,ease:"power2.out"})},this.app.on("homeAnimationStatic",()=>{this.mouseEnabled=!0}),this.init()}init(){this.setMaterial(),this.setMesh(),this.debug()}debug(){if(!this.app.debug.active)return;this.app.debug.gui.addFolder("Home/Hero")}setMaterial(){this.material=new w({vertexShader:P,fragmentShader:M,transparent:!0,depthTest:!1,uniforms:{uTexture:new r(null),uRes:new r(new h(this.sizes.width,this.sizes.height)),uAspect:new r(new h(16,9)),uSize:new r(new h(0,0)),uBorder:new r(0),uTime:new r(0),uFluid:new r(null),uHovered:new r(0),uColor:new r(new R(255/255,118/255,162/255)),uMouse:new r(this.mouse),uOpacity:new r(0)}})}setMesh(){this.group=new S,this.scene.add(this.group),this.meshs=[...this.items].map((e,o)=>{const s=[...this.items].filter(v=>v.querySelector("img")).indexOf(e),i=window.getComputedStyle(e).getPropertyValue("border-radius"),n=e.getBoundingClientRect(),c=/^((?!chrome|android).)*safari/i.test(navigator.userAgent)?20:30,l=new g(n.width,n.height,c,c),a=this.material.clone();a.uniforms.uSize.value.set(n.width,n.height),a.uniforms.uBorder.value=parseFloat(i)||0;const p=e.querySelector("img"),d=e.querySelector("video");if(d)if(d._videoLoaderInstance){const v=d._videoLoaderInstance;if(v.isLoaded){const f=new m(d);a.uniforms.uTexture.value=f,a.uniforms.uAspect.value.set(v.width||d.videoWidth,v.height||d.videoHeight)}else v.on("loaded",()=>{const f=new m(d);a.uniforms.uTexture.value=f,a.uniforms.uAspect.value.set(v.width,v.height)})}else{const v=new b(d,{lazyLoad:!1});v.on("loaded",()=>{const f=new m(d);a.uniforms.uTexture.value=f,a.uniforms.uAspect.value.set(v.width,v.height)})}else if(p){a.uniforms.uTexture.value=this.resources[s];const v=p.getAttribute("src"),f=new Image;f.src=v,f.crossOrigin="anonymous",f.onload=()=>{a.uniforms.uAspect.value.set(f.width,f.height)}}const y=new U(l,a),z=gsap.timeline({paused:!0});return z.to(y.material.uniforms.uHovered,{value:1,duration:.4,ease:"power1.inOut"}),this.group.add(y),e.style.setProperty("opacity","0"),this.app.observer.instance.observe(e),{mesh:y,item:e,material:a,tl:z}}),this.meshs.forEach(({item:e},o)=>{e.addEventListener("mouseenter",()=>{this.meshs.forEach(({tl:t},s)=>{s!=o&&t.play()})}),e.addEventListener("mouseleave",()=>{this.meshs.forEach(({tl:t},s)=>{t.reverse()})})}),this.setPosition()}setPosition(){this.meshs.forEach(({mesh:e,item:o})=>{if(o.dataset.visible=="false")return;const t=o.getBoundingClientRect();e.position.x=t.left+t.width/2-this.sizes.width/2,e.position.y=-t.top-t.height/2+this.sizes.height/2})}onMouseMove(e){this.mouseEnabled&&(this.mouse.x=e.clientX-window.innerWidth/2,this.mouse.y=e.clientY-window.innerHeight/2,this.offset.x=this.lerp(this.offset.x,this.mouse.x,.1),this.offset.y=this.lerp(this.offset.y,this.mouse.y,.1),this.offsetQuicks.x(-(this.mouse.x-this.offset.x)*.1),this.offsetQuicks.y((this.mouse.y-this.offset.y)*.02))}resize(){this.meshs.forEach(({mesh:e,item:o})=>{const t=o.getBoundingClientRect();A(e,new g(t.width,t.height,50,50)),e.material.uniforms.uSize.value.set(t.width,t.height);const s=window.getComputedStyle(o).getPropertyValue("border-radius");e.material.uniforms.uBorder.value=parseFloat(s)||0})}update(){this.meshs.forEach(({mesh:e,material:o})=>{o.uniforms.uFluid.value=this.gl.fluidTexture}),this.offsetQuicks.x(0),this.offsetQuicks.y(0)}destroy(){this.meshs.forEach(({mesh:e,material:o})=>{o.dispose(),e.geometry.dispose(),this.group.remove(e)}),this.group&&this.scene.remove(this.group)}lerp(e,o,t){return e*(1-t)+o*t}};var Y=`varying vec2 vUv;
varying vec2 screenUv;
